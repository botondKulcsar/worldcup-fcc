...
echo "$($PSQL "SELECT unnest(array[t.name, teams.name]) AS country FROM games INNER JOIN teams AS t ON games.opponent_id=t.team_id INNER JOIN teams ON games.winner_id=teams.team_id WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY country")"
